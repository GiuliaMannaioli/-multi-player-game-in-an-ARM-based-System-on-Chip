...
void LCD_DrawBall( uint16_t x0, uint16_t y0, uint16_t color ){
	int i;
	for(i=0;i<5;i++){
    LCD_DrawLine( x0-2, y0-2+i, x0+2, y0-2+i , color);
	}
...
void PutCharInverse( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
	uint16_t i, j;
    uint8_t buffer[16], tmp_char;
    GetASCIICode(buffer,ASCI);  /* È¡×ÖÄ£Êý¾Ý */
...
void GUI_Text(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor)
{
    ...
    do
    {
        TempChar = *str++;  //questo mi prende una lettera per volta 
        PutChar( Xpos, Ypos, TempChar, Color, bkColor );     //xpos da dove parte
        if( Xpos < MAX_X - 8 )
        {
            Xpos += 8; // l'ordine in cui le scrive, cioè dopo 8 pixel
        } 
        else if ( Ypos < MAX_Y - 16 ) //ypos da dove parte
        { 
            Xpos = 0;
            Ypos += 16;
        }   
...
void GUI_TextInverse(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor)
{
    uint8_t TempChar;
    do
    {
        TempChar = *str++;  
        PutCharInverse( Xpos, Ypos, TempChar, Color, bkColor );    
        if( Xpos < MAX_X - 8 )
        {
            Xpos += 8;
        } 
